...
:math:`m`, a charge :math:`q`, and a velocity :math:`\vec{V}`, the force 
:math:`\vec{F}` is given by:

.. math::

   \vec{F}(\vec{r}) = m\vec{V}\left(\vec{r}\right) + 
...
MODULE_RUN(ClassicalForce) {
    const auto& [q, m, v, charges] = force_type::unwrap_inputs(inputs);
    ...
    for(std::size_t i = 0; i < 3; ++i) { F[i] = m * v[i] + q.m_charge * E[i]; }
