 ...
 * ``sde::property_type`` class. ``sde::property_type`` is a templated class
 * that requires one template type parameter, which is the type of the class
 * deriving from it. This is a common pattern in C++ known as the curiously
 * recursive template pattern, or CRTP, that is used to implement static
 * polymorphism. For our purposes, this amounts to the sde seeing the property
 * type API, we are about to define, at compile-time so that type-safety can be
