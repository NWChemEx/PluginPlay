...
/* These are property types used for unit testing the Python bindings, while
 * they have a strong overlap with those in the C++ tests, it's assumed that
 * they will diverge over time as we need to add more property types focused on
 * testing the interface between Python and C++.
 */
