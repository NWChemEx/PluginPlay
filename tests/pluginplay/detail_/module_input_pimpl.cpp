...
TEST_CASE("ModuleInputPIMPL : Default ctor") {
    ...
    REQUIRE_THROWS_AS(pimpl.is_valid(any::make_any_field<double>(double{3})),
                      std::runtime_error);
}
...
TEST_CASE("ModuleInputPIMPL : is_valid") {
    ModuleInputPIMPL p;
    REQUIRE_THROWS_AS(p.is_valid(any::make_any_field<int>(3)),
                      std::runtime_error);
    p.set_type(std::type_index(typeid(int)));

    SECTION("No checks") {
        REQUIRE(p.is_valid(any::make_any_field<double>(1.1)));
    }

    SECTION("Has checks") {
        p.add_check([](const type::any& any) { return any.has_value(); });
        REQUIRE(p.is_valid(any::make_any_field<int>(3)));
        REQUIRE_FALSE(p.is_valid(type::any{}));
    }
}
