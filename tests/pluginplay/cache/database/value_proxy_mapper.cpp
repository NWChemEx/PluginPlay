...
#include <map>
#include <pluginplay/cache/database/value_proxy_mapper.hpp>
...
TEMPLATE_LIST_TEST_CASE("ValueProxyMapper", "", testing::test_types) {
    ...
    // This is the value the ProxyMapMaker mapped value0 to, since that class is
    // known to work we don't have to test that it's: {{"foo", <uuid>}}
    auto mapped_value0 = pmapper->at(value0);
    ...
    SECTION("insert/at") {
        ...
        // Value is stored in a proxied format
        REQUIRE(psub_db->at(key0).get() == mapped_value0);
        ...
        REQUIRE(psub_db->at(key1).get() == pmapper->at(value1));
    }
    ...
    SECTION("free") {
        ...
        REQUIRE_FALSE(psub_db->count(key0));
    ...
    SECTION("backup") {
        ...
        // Still in sub_db
        REQUIRE(psub_db->count(key0));
        // Check that we called backup on psub_db
        REQUIRE(psub_sub_db->count(key0));
    }
