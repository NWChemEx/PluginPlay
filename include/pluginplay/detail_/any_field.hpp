...
/** @brief The AnyField class is capable of holding an instance of any type
 ...
 * the additional functions which are part of the AnyField's API, but not
 * the std::any's API.
 */
class AnyField {
private:
    /// Trait to see if @p T is an AnyField
    template<typename T>
    using is_any_field = std::is_same<AnyField, std::decay_t<T>>;

    /// True if @p T is an AnyField
    template<typename T>
    static constexpr bool is_any_field_v = is_any_field<T>::value;

    /// True if @p T is not an AnyField
    template<typename T>
    static constexpr bool not_an_any_field_v = std::negation_v<is_any_field<T>>;

    /// Enables a function if @p T is an AnyField
    template<typename T>
    using enable_if_any_field_t = std::enable_if_t<is_any_field_v<T>, int>;

    /// Enables a function if @p T is not an AnyField
    template<typename T>
    using enable_if_not_an_any_field_t =
      std::enable_if_t<not_an_any_field_v<T>, int>;
...
public:
    ...
    /** @brief Makes an empty AnyField instance.
     *
     *  The resulting AnyField instance wraps no object.  An object can be
     * added to this instance by calling the member function emplace, by
     * assigning to this instance another AnyField instance (containing a
     * value), or by moving from another AnyField instance containing a
     * value.
     *
    ...
    /** @brief Used to construct an AnyField instance holding a particular
     * value.
     *
     ...
     *             as @p T's ctor.
     */
    template<typename T, enable_if_not_an_any_field_t<T> = 0>
    explicit AnyField(T&& value);

    /**
     * @brief Makes a new AnyField instance by deep copying the instance
     * wrapped in another AnyField instance.
     *
     ...
     *
     * @param[in] rhs The AnyField instance to copy.
     *
     ...
     */
    AnyField(const AnyField& rhs) : m_ptr_(rhs.clone_()) {}
     ...
     * guarantee.
     */
    AnyField& operator=(const AnyField& r) {
        return *this = std::move(AnyField(r));
    }

    /**
     * @brief Causes the current AnyField instance to take ownership of
     * another instance.
     *
     * @param[in] rhs The AnyField instance to take ownership of. After
     ...
     */
    AnyField(AnyField&& rhs) noexcept = default;

    /**
     * @brief Sets the current instance to the state of another AnyField
     * instance.
     *
     * @param[in] rhs The AnyField instance we are taking the state of.
     ...
     * @throw None. No throw guarantee.
     */
    AnyField& operator=(AnyField&& rhs) noexcept = default;

    /** @brief Frees up the memory wrapped by the AnyField instance.
     *
     ...
     * action.  Let's say you have an AnyField instance and you want it to
     * wrap a double, this could be done like:
     *
     * @code
     * AnyField my_any_field;
     * double& wrapped_value = my_any_field.emplace<double>(3.14);
     * @endcode
     *
     ...
     * auto& wrapped_ve1 = my_any_field.emplace<std::vector<double>>(vec1);
     *
     * // ...makes vec1, inside the AnyField, without the copy
     * auto& wrapped_v2 =
     * my_any_field.emplace<std::vector<double>>({1.1, 1.2, 1.3});
     * @endcode
     *
     ...
     *         it.
     *
     *  AnyField instances own the memory they are wrapping.  This member
     * function can be used to release the memory before the AnyField
     * instance goes out of scope. Following normal RAII practices the
     * AnyField will release any held memory in its dtor meaning users need
     * not call this function before letting an AnyField instance go out of
     * scope.
     *
     * @throw None. No throw guarantee.
     */
    ...
    /** @brief Swaps the states of two AnyField instances.
     *
     ...
     *
     * @param[in,out] rhs The AnyField instance to swap contents with.
     ...
     */
    void swap(AnyField& rhs) noexcept { m_ptr_.swap(rhs.m_ptr_); }

    /** @brief Used to retrieve the value wrapped in the AnyField.
     *
     *  The canonical way to retrieve the value from an AnyField is to use
     *  AnyFieldCast. This function is the implementation for the cast and
     * can be called directly if the user wants.
     *
     * @tparam T The type to cast the wrapped AnyField instance to. Should
    ...
    /** @brief Used to retrieve the value wrapped in a read-only AnyField.
     *
     *  The canonical way to retrieve the value from an AnyField is to use
     *  AnyFieldCast. This function is the implementation for the cast and
    ...
    /** @brief Returns true if the current AnyField instance is presently
     * wrapping a value.
     *
     *  An AnyField can either be holding a type-erased value or not. This
     ...
     *         instance.
     *
     *  This function ultimately works by printing "<empty AnyField>" if
    ...
    /** @brief Compares two AnyField instances for equality.
     *
     * Two AnyField instances are equal if they both hold a value (or do
     ...
     *
     * @return true if the AnyField instances are equal and false
     * otherwise.
     *
    ...
    /** @brief Compares two AnyField instances to see if they are
     * different.
     *
     * Two AnyField instances are equal if they both hold a value (or do
     * not hold a value) and, in the case that they both hold a value, the
     ...
     * @param[in] rhs The instance to compare to
     *
     * @return false if the AnyField instances are equal and true
     * otherwise.
     *
     * @throw None. All comparisons are no throw guarantee.
     */
...
private:
     ...
     *  internal wrapper.
     *
     * @tparam T The cv qualified type that this AnyField instance will be
     *           wrapping.
     *
     ...
     *
     * @return A unique_ptr containing an AnyFieldWrapperfield instance
...
/** @brief Provides access to the value wrapped in an AnyField instance.
 *
 * @relates AnyField
 *
 * @tparam T The type of the value wrapped in the AnyField instance.
 * @tparam U The type of the AnyField instance itself. Should be at most
 *           cv-qualified AnyField.
 *
 * @param[in] da_any_field The AnyField instance to retrieve the value from.
 *
 * @return The value wrapped by @p da_any_field.
 *
 * @throw std::bad_any_cast if the value wrapped by @p da_any_field is not
 *                          convertible to type @p T or if @p da_any_field does
 * not contain a value.  Strong throw guarantee.
 *
 * @par Complexity: Constant.
 */
template<typename T, typename U>
T AnyFieldCast(U&& da_any_field) {
    return da_any_field.template cast<T>();
}

/** @brief Makes an AnyField instance by forwarding the arguments to the
 * wrapped instance's constructor.
 *
 *  @relates AnyField
 *
 *  This is a convenience function for directly populating an AnyField
 * instance so that the user does not have to first construct a temporary and
 * then forward that temporary to an AnyField instance.
 *
 *  @tparam T The type of the object that the AnyField should wrap.
 *  @tparam Args The types of the arguments that are being forwarded.
 *
 *  @param[in] args The values to forward to @p T's ctor.
 *
 *  @return An AnyField instance containing a newly created wrapped @p T
 * instance.
 *
 *  @par Data Races:
 ...
 *
 *  @throw std::bad_alloc if there is insufficient memory for the AnyField
 ...
 */
template<typename T, typename... Args>
AnyField make_AnyField(Args&&... args);

//--------------------------Implementations-------------------------------------

template<typename T, typename AnyField::enable_if_not_an_any_field_t<T>>
AnyField::AnyField(T&& value) :
  m_ptr_(wrap_ptr_<std::conditional_t<std::is_reference_v<T>,
                                      std::remove_reference_t<T>, T>>(
    std::forward<T>(value))) {}

template<typename T, typename... Args>
T& AnyField::emplace(Args&&... args) {
...
inline typename AnyField::rtti_type AnyField::type() const noexcept {
    return has_value() ? m_ptr_->type() : typeid(void);
}

inline std::string AnyField::str() const {
    if(has_value()) return m_ptr_->str();
    return "<empty AnyField>";
}

inline bool AnyField::operator==(const AnyField& rhs) const noexcept {
...
inline typename AnyField::wrapper_ptr AnyField::clone_() const {
    return has_value() ? m_ptr_->clone() : wrapper_ptr{};
}

template<typename T, typename... Args>
typename AnyField::wrapper_ptr AnyField::wrap_ptr_(Args&&... args) {
    static_assert(!std::is_reference_v<T>, "T should not be a reference");

    T temp(std::forward<Args>(args)...);
    return std::make_unique<AnyFieldWrapper<T>>(std::move(temp));
}
