 ...
 * std::any to.
 */
class AnyFieldWrapperBase {
protected:
    /// My type
    using my_type = AnyFieldWrapperBase;
...
public:
    ...
    using wrapper_ptr = std::unique_ptr<AnyFieldWrapperBase>;

    /// Type of the RTTI of the wrapped instance
    using rtti_type = const std::type_info&;

    /** @brief Creates an AnyFieldWrapper by forwarding the provided value.
     *
     *  @note This ctor only participates in overload resolution if @p T is
     * not AnyFieldWrapperBase or any instanitation of
     * AnyFieldWrapper.
     *
    ...
    explicit AnyFieldWrapperBase(T&& value) :
      m_value_(std::forward<T>(value)) {}

    /** @brief Cleans up an AnyField_ instance.
     *
     *  Since AnyField_ instances have no state this is a null
     ...
     */
    virtual ~AnyFieldWrapperBase() = default;
     ...
     *  @throw none No throw guarantee.
     */
    bool operator==(const AnyFieldWrapperBase& rhs) const noexcept;
...
protected:
     ...
     */
    AnyFieldWrapperBase(const AnyFieldWrapperBase&) = default;
     ...
     *  @throw none No throw guarantee.
     */
    AnyFieldWrapperBase(AnyFieldWrapperBase&&) noexcept = default;
     ...
     *  @throw ??? if the copying the value throws. Same guarantee.
     */
    AnyFieldWrapperBase& operator=(const AnyFieldWrapperBase&) = default;
     ...
     *  @throw none No throw guarantee.
     */
    AnyFieldWrapperBase& operator=(AnyFieldWrapperBase&&) noexcept = default;
...
private:
    ...
    virtual bool are_equal_(const AnyFieldWrapperBase& rhs) const noexcept = 0;
 ...
 *
 * The AnyFieldWrapper class holds the wrapped instance for the
 * AnyFieldWrapperBase class.  It also is responsible for implementing all
 * of the abstract methods
 *
 *
 * @tparam T The type of the instance to wrap. Must be copyable
 * and have operator== defined.
 */
template<typename T>
class AnyFieldWrapper : public AnyFieldWrapperBase {
private:
...
public:
    /**
     * @brief Creates a new AnyFieldWrapper with the provided value
     *
     * @param[in] value The value we are wrapping.
     *
    ...
    explicit AnyFieldWrapper(U&& value) : base_type(std::forward<U>(value)) {}
...
protected:
     ...
     *  @throw ??? if the copying the value throws. Same guarantee.
     */
    AnyFieldWrapper(const AnyFieldWrapper<T>& rhs) = default;
     ...
     *  @throw none No throw guarantee.
     */
    AnyFieldWrapper(AnyFieldWrapper<T>&&) noexcept = default;
     ...
     *  @throw ??? if the copying the value throws. Same guarantee.
     */
    AnyFieldWrapper<T>& operator=(const AnyFieldWrapper<T>&) = default;
     ...
     *  @throw none No throw guarantee.
     */
    AnyFieldWrapper<T>& operator=(AnyFieldWrapper<T>&&) noexcept = default;
...
private:
     ...
     *
     *  @return A newly allocated AnyField_ instance.
     *
...
template<typename T>
bool AnyFieldWrapperBase::is_convertible() const noexcept {
    return std::any_cast<T>(&m_value_) != nullptr;
}

inline bool AnyFieldWrapperBase::operator==(
...
template<typename U>
explicit AnyFieldWrapper(U&& value)
  -> AnyFieldWrapper<std::remove_reference_t<U>>;

template<typename T>
typename AnyFieldWrapper<T>::wrapper_ptr AnyFieldWrapper<T>::clone_() const {
    return std::make_unique<AnyFieldWrapper<T>>(value_());
}

template<typename T>
std::string AnyFieldWrapper<T>::str_() const {
    std::stringstream ss;
...
template<typename T>
bool AnyFieldWrapper<T>::are_equal_(
  const AnyFieldWrapperBase& rhs) const noexcept {
    try {
