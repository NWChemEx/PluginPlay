 ...
 *  defined by the SDE. The sde::bounds_check::detail_ namespace includes the
 *  machinery required for these checks.
 */
namespace detail_ {
 ...
 *
 *  If a bounds check functor is one of the SDE's built in bounds checks we can
 *  just call its `str` method to get a string representation of the check. To
 *  determine if we can call `str` we rely on the helper class
